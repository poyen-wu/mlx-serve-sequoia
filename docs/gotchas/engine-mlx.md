...
### A Metal COMPILER internal error is a machine property — probe custom kernels, self-decline (2026-09-26)
A G16s / macOS 15.8 box could not compile the `msv_attn_p256` specialization at all (`metal::Device Unable to load kernel … Compiler encountered an internal error`): the first hd-256 prefill chunk latched `MlxFailure` and EVERY request on that model failed with `GenerationFailed`, because the compile only happens at the first eval of the first real dispatch. Fix (`transformer.zig` `fused256ArmProbe`): the first time the kernel is about to be applied — including under the test override — a one-shot probe evaluates the exact causal specialization on tiny synthetic LCG arrays, with the latching error handler installed (tests never install it; without this the failure hits mlx-c's default `exit(-1)` and the test binary dies). A failed probe permanently self-declines the kernel at the DISPATCH site only (`fusedSdpa256Impl` returns null → composed sdpa serves); the MODE and BILLING predicates (`fused256CausalMode`, `prefillHeadDimFused`) are deliberately untouched, so admission guards keep pricing the fused world and the pinned-number tests do not move. The probe takes its own `takeError` so the latch never leaks into the next request. Guards: `a failed compile probe permanently declines the fused hd-256 kernel` (red-on-revert: it returns the array instead of null); the six forced-engagement `fusedSdpa256*` tests skip via `fused256ArmProbe() + SkipZigTest` on a compile-broken machine instead of crashing with exit 255. Rule class: any `metal_kernel` that compiles lazily at first use owes a one-shot compile probe with a permanent runtime decline; the decline must gate DISPATCH, never the predicates that bill or route.
